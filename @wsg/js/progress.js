...
function progressList() {
	...
	var myProject = myPath.replace("/@dari","/html");
	...
	stateFields.each(function(i, v) {
		...
		var prop = {
			...
			pageIdQuery: fields.find('.pageid[data-query]').data("query")
		};

		var wrapAnchor = $("<a>")
			.attr("target", "_blank")
			.attr("href", myProject +"/"+ prop.directory + "/" + prop.pageId + ".html" + (prop.pageIdQuery ? prop.pageIdQuery : ""))
			.text(v.textContent);
